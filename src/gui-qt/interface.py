...
class interface:
	...
	def position(self):
		...
		for index, obj_element in enumerate(self.screen_elements):
			...
			if not pos_y == -1:
				current_heigth = pos_y
			element.setGeometry(pos_x, current_heigth, element.width() if (size[0] == None) else size[0], element.height() if (size[1] == None) else size[1])
			#	this is given to the window, we want to make sure everything fits
			width_size = max(element.width(), width_size)
			height_size = max(current_heigth, height_size)
			#	increasing the size so the next element comes after the current
			current_heigth += element.height() if(0 < index) else 0
