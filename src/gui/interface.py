...
class interface_class(App):
    ...
    def position(self):
        ...
        for index, obj_element in enumerate(self.screen_elements):
            if obj_element.free_floating:
                obj_element.element.setGeometry(0, 30, obj_element.size_element.width(),
                                                obj_element.size_element.height())
                continue
    ...
    def add_slider(self, text, action: Callable, value: int = -1, max_value=100):
        """
        Creates a QSlider
    ...
    def add_radio_buttons(self, text, button_text, action, enabled, disabled_box=False):
        """
        Creates a QPushButton
        ...
        action : lambda
            the action to run when a new button is toggled
        enabled: int
            the button index to have enabled from the start
        disabled_box : bool
            if the entire box should be disabled
        ...
        for index, i in enumerate(button_text):
            ...
            if index == enabled:
                i.setChecked(True)
            i.setDisabled(disabled_box)
        box.setLayout(button_layout)

        self.screen_elements.append(screen_element(box))
        return box, mood_button_group

    def add_input_group(self, text, info, action):
        """
        Creates a QGroupBox with input boxes

        Parameters
        ----------
        text : str
            the text to set for the box
        info : list
            the text to be set to a box
        action : lambda
            the action to run when a new button is toggled
