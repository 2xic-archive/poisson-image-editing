...
def test_rest(_qt_bot_, interface_app, setup_code=lambda x, y, z: 0):
    """
    ...
    _qt_bot_.wait(100)
    _qt_bot_.waitUntil(
        lambda: (interface_app.method.data.shape != current_image.shape) or not np.allclose(interface_app.method.data,
                                                                                            current_image),
        timeout=3000)
    ...
    for index, i in enumerate(interface_app.screen_elements):
        assert (elemenets[
                    index] == i.element.isEnabled()), "error -> state is not the same after reset (index id: {}, {})".format(
            index, i.element.text())
    interface_app.close()
